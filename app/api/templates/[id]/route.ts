...
export async function GET(
  ...
  try {
    ...
    const pages: PageResponse[] = template.canvases.map((canvas) => ({
      ...
      layers: canvas.placeholders.map(
        (p: TemplatePlaceholder): LayerResponse => ({
          id: p.id,
          name: p.name,
          ...
          // Position (ratios)
          xRatio: p.x_ratio || undefined,
          yRatio: p.y_ratio || undefined,
          widthRatio: p.width_ratio || undefined,
          heightRatio: p.height_ratio || undefined,

          // Anchors
          anchorH: p.anchor_h,
          anchorV: p.anchor_v,
          isBackground: p.is_background,
